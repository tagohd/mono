...
Formula for maximum instantaneous mana: $$m=\frac{n e^n L_E}{E S^{|n-E|}}$$ where $m$ is the Mana cost, $n$ is the number of souls, $e$ is Euler's number, $L_E$ is Ejiofor's limit (1.21 PEj), $E$ is Ejiofor's constant ($6.3459269958\times10^{-3}$), and $S$ is the Stokoff constant (39.810686024).
